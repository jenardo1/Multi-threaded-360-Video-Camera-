...
def main():
    ...
    NUM_CAMERAS = 7  # <-- change this number # 4

    # Initialize cameras
    cams = [cv.VideoCapture(i) for i in range(NUM_CAMERAS)]
    for cam in cams:
        #cam.set(cv.CAP_PROP_FRAME_WIDTH, 1280)  # or your preferred size
        #cam.set(cv.CAP_PROP_FRAME_HEIGHT, 720)
        #cam.set(cv.CAP_PROP_FPS, 30)
        print(f"FPS: {cam.get(cv.CAP_PROP_FPS)}")

    # Synchronize capture
    frames = []

    print("[INFO] Warming up cameras for 2 seconds...")
    for _ in range(60):  # 60 iterations x ~33ms ≈ 2 seconds
        for cam in cams:
            cam.read()
        cv.waitKey(33)  # Wait ~33ms between reads (≈ 30 FPS)

    # Grab frames as close to the same moment as possible
    for idx, cam in enumerate(cams):
        ret, frame = cam.read()
        if not ret:
            print(f"[ERROR] Failed to capture frame from camera {idx}")
            for c in cams:
                c.release()
            exit()
        frames.append(frame)
    ...
    # Resize the frame
        #resized_frame = cv.resize(frame, None, fx=1, fy=1, interpolation=cv.INTER_LINEAR)

    # Release cameras
    for cam in cams:
        cam.release()

    print(f"[INFO] Successfully captured {len(frames)} synchronized frames for precompute.")\

    work_megapix = 0.1 #0.6 #args.work_megapix
    ...
    widthstretch = 0.85 # 1 0.78
    for idx, full_img in enumerate(frames):
        if full_img is None:
            print("Cannot read image at camera {idx}")
            exit()
        ...
        if work_megapix < 0:
            ...
            if is_work_scale_set is False:
                ...
                is_work_scale_set = True
# HERE
            img = cv.resize(src=full_img, dsize=None, fx=widthstretch*work_scale, fy=work_scale, interpolation=cv.INTER_LINEAR_EXACT)
        ...
        img = cv.resize(src=full_img, dsize=None, fx=widthstretch*seam_scale, fy=seam_scale, interpolation=cv.INTER_LINEAR_EXACT)
        images.append(img)
...
# Perform feature matching
    ...
    for i in range(len(indices)):
        frames_subset.append(frames[indices[i]])
    ...
    for idx, full_img in enumerate(frames):
        ...
        if abs(compose_scale - 1) > 1e-1:
            img = cv.resize(src=full_img, dsize=None, fx=widthstretch*compose_scale, fy=compose_scale,
        ...
        if timelapse:
            ...
            fixed_file_name = f"fixed_camera_{idx}.jpg"
            cv.imwrite(fixed_file_name, timelapser.getDst())
            print(f"[INFO] Saved timelapse frame: {fixed_file_name}")
        else:
